...
var templates = map[Code]string{
	...
	ErrSubResourceExist:        "The {{if .type}}({{.type}}) {{end}}resource{{if .name}} ({{.name}}){{end}} exist",
	...
	ErrDataTooLarge:    "Data too large. Resource {{if .name}}({{.name}}){{end}}, size={{if .size}}({{.size}}){{end}}, max={{if .max}}({{.max}}){{end}}",
}
